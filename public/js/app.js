...
// 加载并显示数据
async function loadAndDisplayData(forceRefresh = false) {
  // 显示加载动画
  uiRenderer.showLoadingAnimation();
  
  try {
    const result = await dataManager.fetchNavigationData(forceRefresh);
    
    if (result.success) {
      // 隐藏加载动画
      uiRenderer.hideLoadingAnimation();
      uiRenderer.setMeta(result);
      
      // 生成分类菜单
      uiRenderer.generateCategoryMenu();
      uiRenderer.updateCategoryChips();
      uiRenderer.updateHeroStats();
      uiRenderer.updateSearchHints();
      uiRenderer.renderFeaturedLinks(forceRefresh);
      uiRenderer.updateGreeting();
      
      // 显示所有工具
      const targetCategory = uiRenderer.getCurrentCategory() || 'all';
      uiRenderer.showTools(targetCategory);
      
      if (uiRenderer.hasActiveSearch()) {
        uiRenderer.filterToolsByKeyword(uiRenderer.getSearchKeyword());
      }
...
function bindEventListeners() {
  ...
  if (heroRefreshBtn) {
    heroRefreshBtn.addEventListener('click', async () => {
      ...
      heroRefreshBtn.setAttribute('aria-busy', 'true');
      await loadAndDisplayData(true);
      heroRefreshBtn.disabled = false;
      heroRefreshBtn.removeAttribute('aria-busy');
    });
  }
